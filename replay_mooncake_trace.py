#!/usr/bin/env python3
"""
Script to replay Mooncake trace using vLLM benchmark serving.
...
class MooncakeTraceDataset:
    """Custom dataset that generates requests based on Mooncake trace."""
        ...
        with open(self.trace_file, 'r') as f:
            for line in f:
                if line.strip():
                    ...
                    timestamp = entry['timestamp'] / 1000.0  # Convert ms to seconds
                    
                    # Filter by time window
                    if (timestamp >= self.start_time and 
                        timestamp <= self.start_time + self.duration):
    ...
    def to_sample_requests_with_timing(self, tokenizer) -> List[Dict[str, Any]]:
        ...
        first_timestamp = self.requests[0]['timestamp'] / 1000.0
...
async def replay_trace(args):
    """Main function to replay the Mooncake trace."""
...
def main():
    parser = argparse.ArgumentParser(description="Replay Mooncake trace with vLLM benchmark")
